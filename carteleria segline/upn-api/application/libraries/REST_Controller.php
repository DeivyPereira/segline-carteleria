...
/**
 * Respuestas de API BEST PRACTI Evenbrit
 *
 *  NOT_FOUND: La URL que proporcionaste no es válida. Código de estado 404.
 *
 * • INVALID_AUTH: Tu autenticación (habitualmente token OAuth) no es correcta. Código de estado 400.
 *
 * • INVALID_AUTH_HEADER: Tu encabezado de autenticación está distorsionado. Código de estado 400.
 *
 * • NO_AUTH: No proporcionaste ninguna autenticación en absoluto. Código de estado 401.
 *
 * • BAD_PAGE: El número de página que proporcionaste no existe o no tiene sentido (por ejemplo, negativo). Código de estado 400.
 *
 * • NOT_AUTHORIZED: Te estás conectando como un usuario que no tiene autorización para ver o realizar esa acción. Código de estado 403.
 *
 * • METHOD_NOT_ALLOWED: Enviaste un método al punto final que no puede manejar (por ejemplo, DELETE a un punto final GET-only). Código de estado 405.
 *
 * • HIT_RATE_LIMIT: Alcanzaste tu límite de frecuencia por hora para este token en la API. Los límites de frecuencia predeterminados se establecen en 1000 llamadas por hora, pero puedes solicitar un aumento del límite de frecuencia de API. Código de estado 429.
 *
 * • INTERNAL_ERROR: Se produjo un error no controlado en Eventbrite. Contacta con la asistencia para desarrolladores si sigue sucediendo. Código de estado 500.
 *
 * • EXPANSION_FAILED: Hubo un error no controlado al tratar de expandir una de las expansiones especificadas. Es probable que la solicitud tenga éxito si no solicitas expansiones, pero contacta con la asistencia técnica para desarrolladores si sigue sucediendo. Código de estado 500.
 *
 * • INVALID_BATCH: Tu solicitud por lotes estaba mal formada o faltaba. Código de estado 400.
 *
 */
abstract class REST_Controller extends CI_Controller
{
    ...
    public function __construct($config = 'rest')
    {
        ...
        if ($config == "key") {
            $this->token = $this->authorization_token->validateToken();
            if ($this->token['status'] == false) {
                $this->response($this->token, REST_Controller::HTTP_OK);
            }
            //Deberia hacerse comprobación del usuario y luego dar datos.
     ...
     */
    public
    function set_response($data = NULL, $http_code = NULL)
    {
     ...
     */
    protected
    function _get_default_output_format()
    {
     ...
     */
    protected
    function _detect_output_format()
    {
     ...
     */
    protected
    function _check_limit($controller_method)
    {
...
// INPUT FUNCTION --------------------------------------------------------------
     ...
     */
    public
    function options($key = NULL, $xss_clean = NULL)
    {
        if ($key === NULL) {
     ...
     */
    public
    function head($key = NULL, $xss_clean = NULL)
    {
        if ($key === NULL) {
     ...
     */
    public
    function put($key = NULL, $xss_clean = NULL)
    {
        if ($key === NULL) {
     ...
     */
    public
    function delete($key = NULL, $xss_clean = NULL)
    {
        if ($key === NULL) {
     ...
     */
    public
    function patch($key = NULL, $xss_clean = NULL)
    {
        if ($key === NULL) {
     ...
     */
    public
    function query($key = NULL, $xss_clean = NULL)
    {
        if ($key === NULL) {
     ...
     */
    protected
    function _xss_clean($value, $xss_clean)
    {
...
// SECURITY FUNCTIONS ---------------------------------------------------------
     ...
     */
    protected
    function _perform_ldap_auth($username = '', $password = NULL)
    {
        if (empty($username)) {
     ...
     */
    protected
    function _check_blacklist_auth()
    {
     ...
     */
    protected
    function _check_whitelist_auth()
    {
     ...
     */
    protected
    function _log_response_code($http_code)
    {
        if ($this->_insert_id == '') {
            return false;
        }
