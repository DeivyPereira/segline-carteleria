        ...
        if ($token_data['status'] === TRUE) {
            try {
                ...
                try {
                    ...
                    return ['status' => FALSE, 'message' => $e->getMessage(), 'code' => 50008];
                }
                ...
                if (!empty($token_decode) and is_object($token_decode)) {
                    ...
                    if (empty($token_decode->id) or !is_numeric($token_decode->id)) {
                        ...
                        return ['status' => FALSE, 'message' => 'Token Time Not Define!', 'code' => 50008];
                    } else {
                        /**
                        ...
                        if ($time_difference >= $this->token_expire_time) {
                            return ['status' => FALSE, 'message' => 'Token Time Expire.', 'code' => 50008];
                        } else {
                            /**
            ...
            return ['status' => FALSE, 'message' => $token_data['message'], 'code' => 50008];
        }
    }
    ...
    public function validateTokenPost()
    {
        if (isset($_POST['token'])) {
            ...
            if (!empty($token) and is_string($token) and !is_array($token)) {
                ...
                return ['status' => FALSE, 'message' => 'Token is not defined.', 'code' => 50008];
            }
        } else {
            return ['status' => FALSE, 'message' => 'Token is not defined.', 'code' => 50008];
        }
